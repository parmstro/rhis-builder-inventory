...
_ssh="ssh -t -o StrictHostKeyChecking=no -o ConnectTimeout=15 -i ${SSH_KEY}"
...
echo ""
echo "  ─────────────────────────────────────────────────────────────────"
echo "  BEFORE building — verify the following prerequisites:"
echo "  ─────────────────────────────────────────────────────────────────"
echo ""
echo "  !! VAULT: An encrypted vault file must exist at:"
echo "       ~/rhis/rhis-builder-inventory/deployments/${DEPLOYMENT}/vault/rhis_builder_vault.yml"
echo "     See vault_SAMPLES/ for the required variables. The vault password"
echo "     must have been transported separately from the transfer drive."
echo ""
echo "  !! MANIFEST: A Red Hat subscription manifest ZIP must exist at:"
echo "       ~/rhis/rhis-builder-inventory/deployments/${DEPLOYMENT}/files/"
echo "     Download from https://access.redhat.com/management — the highside"
echo "     Satellite cannot reach the CDN to generate one."
echo ""
echo "  ─────────────────────────────────────────────────────────────────"
