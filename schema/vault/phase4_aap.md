...
| `aap_controller_manifest_zip_vault` | notsecret | The filesystem path where the AAP Controller manifest ZIP will be staged inside the container. Default is `/tmp/rhis_aap_manifest.zip`. | Set to a writable path inside the container. | rhis-builder-aap |
...
| `aap_admin_username_vault` | notsecret | AAP Controller administrator username. Aliased to `default_admin_username_vault` in POC environments. | Set to your chosen AAP admin username. | rhis-builder-aap |
...
| `aap_auth_ldap_bind_dn_vault` | notsecret | The LDAP bind DN used by AAP Controller to query IdM for user authentication. Typically a dedicated service account in IdM. | Create a service account in IdM and use its full DN (e.g. `uid=ldap-lookup,cn=users,cn=accounts,dc=example,dc=ca`). | rhis-builder-aap |
...
| `private_hub_username_vault` | notsecret | Administrator username for the Private Automation Hub. Aliased to `default_admin_username_vault`. | Set to your chosen Private Hub admin username. | rhis-builder-aap, rhis-builder-day-2-ops |
...
| `private_automation_hub24_token_vault` | secret | API token generated for a standalone AAP 2.4 Private Automation Hub instance. Used for collection synchronization. | Generate via the Private Hub UI under Collections → API token. | rhis-builder-day-2-ops |
...
| Variable | Classification | Description | How to obtain | Used by |
|---|---|---|---|---|
| `ansible_github_username_vault` | notsecret | GitHub username for the automation account used by AAP to clone repositories. | Set to the GitHub account that owns the RHIS repositories. | rhis-builder-aap |
...
| `github_demo_username_vault` | notsecret | GitHub username for the account used to pull and push demo project content. | Set to the GitHub account used for demo repositories. | rhis-builder-day-2-ops |
...
| Variable | Classification | Description | How to obtain | Used by |
|---|---|---|---|---|
| `aap_idm_machine_username_vault` | notsecret | Username for the AAP machine credential used to connect to IdM-managed hosts. Aliased to `default_admin_username_vault`. | Set to the user account present on managed hosts. | rhis-builder-aap |
