...
| `piv_secure_dir_vault` | notsecret | The filesystem path on the provisioner where YubiKey PIV certificates and keys are staged during configuration. Default is `/root/yubico`. | Set to a directory accessible only by root on the provisioner. | rhis-builder-yubi |
| `piv_vault_dir_vault` | notsecret | The filesystem path on the provisioner where the PIV vault files (encrypted key material) are stored. Default is `/root/vault`. | Set to a directory accessible only by root on the provisioner. | rhis-builder-yubi |
