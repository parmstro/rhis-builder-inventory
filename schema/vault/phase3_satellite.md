...
|---|---|---|---|---|
| `satellite_username_vault` | notsecret | Satellite administrator username. Aliased to `default_admin_username_vault` in POC environments. | Set to your chosen Satellite admin username. | rhis-builder-satellite |
...
| `cdn_account_number_vault` | notsecret | Your Red Hat account number. | Found on the Red Hat Customer Portal under your account profile. | rhis-builder-satellite |
| `cdn_organization_id_vault` | notsecret | Your Red Hat organization ID (may differ from account number in multi-org environments). | Found on the Red Hat Customer Portal under your organization settings. | rhis-builder-satellite |
| `cdn_sat_activation_key_vault` | notsecret | The activation key name used specifically for Satellite CDN registration. | Create an activation key configured for Satellite on the Customer Portal or console.redhat.com. | rhis-builder-satellite |
| `manifest_subs_vault` | notsecret | A list of subscription pool IDs and quantities to include in the Satellite manifest. Each entry requires `name`, `pool_id`, and `qty`. | Find pool IDs via `hammer subscription list --organization <org>` or the Customer Portal. | rhis-builder-satellite |
...
| Variable | Classification | Description | How to obtain | Used by |
|---|---|---|---|---|
| `ipa_keytab_dn_vault` | notsecret | The LDAP distinguished name used to pull an existing Kerberos keytab from IdM. Defaults to `cn=Directory Manager`. Only the DM can pull an existing user keytab by default. | Set to an account with sufficient IdM directory permissions. | rhis-builder-satellite |
...
| Variable | Classification | Description | How to obtain | Used by |
|---|---|---|---|---|
| `remote_execution_ssh_keys_vault` | secret | The SSH public key(s) that Satellite will distribute to managed hosts for remote execution. | Use the public key from the Satellite host's SSH key pair, or generate a dedicated REX key pair. | rhis-builder-satellite |
| `remote_execution_ssh_user_vault` | notsecret | The username Satellite uses for SSH remote execution connections. Aliased to `rhis_builder_default_user`. On AWS instances this is typically `ec2-user`. | Set to the username present on managed hosts that allows SSH access. | rhis-builder-satellite |
...
| Variable | Classification | Description | How to obtain | Used by |
|---|---|---|---|---|
| `vcenter_virtwho_username_vault` | alias | vCenter username for virt-who. Aliased to `vcenter_service_username_vault`. Use a dedicated service account in production. | — | rhis-builder-satellite |
...
| `satellite_virtwho_username_vault` | alias | Satellite username that virt-who uses to update Satellite with discovered VMs and hypervisors. Aliased to `satellite_username_vault`. | — | rhis-builder-satellite |
...
| Variable | Classification | Description | How to obtain | Used by |
|---|---|---|---|---|
| `satellite_smtp_user_name_vault` | notsecret | The SMTP username for Satellite email notifications. Convention is `satellite@<domain>`. | Set to the email account Satellite will send from. | rhis-builder-satellite |
