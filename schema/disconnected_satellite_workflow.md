...
The `satellite_import_content` flag controls whether the `content_imports` role runs. It is set to `true` exclusively by `build_sat_disconnected_import.sh` in the extra-vars file it writes before calling `main.yml`. It is **not** set in the deployment inventory.

There are three distinct post-install operational workflows, each using a different entry point:

| Workflow | Entry point | `satellite_import_content` |
|---|---|---|
| **Initial build or incremental import** | `build_sat_disconnected_import.sh` | `true` (set by the script) |
| **Config-only re-run** (no new content) | `main.yml` directly with disconnected vars | unset — import skipped |
| **Incremental import** (new content cycle) | `build_sat_disconnected_import.sh` with new bundle | `true` (new bundle metadata → new CV version) |

**Config-only re-run** — use `build_sat_disconnected_import.sh --config-only` after the initial build when you need to change provisioning templates, operating system definitions, hostgroups, or any other configuration without re-importing content:
...
The `--config-only` flag skips bundle delivery and the extra-vars file entirely, then runs `main.yml` with `--tags tags_post_sync`, which bypasses both the satellite-installer and the `content_imports` role. Expected duration: 10–20 minutes.

**Incremental import** — when a new content cycle is ready, update `content_imports.yml` to point to the new bundle path, then run `build_sat_disconnected_import.sh` as normal. The new bundle carries new Pulp metadata, so Satellite creates a new content view version and the import succeeds.
...
| `satellite_import_content` | (not applicable) | Set by `build_sat_disconnected_import.sh`; not in inventory. Omit for config-only runs. |
...
### 8.6 Post-build operational workflows

`satellite_import_content` is set to `true` only by `build_sat_disconnected_import.sh` and is never stored in the deployment inventory. This means the three post-build workflows are naturally separated by entry point rather than by a flag the operator must remember to flip:

**Initial build / incremental import** — use `build_sat_disconnected_import.sh`. The script delivers the bundle, writes the extra-vars file with `satellite_import_content: true`, and calls `main.yml`. For an incremental import, update `content_imports.yml` to point to the new bundle path before running the script; the new bundle carries different Pulp metadata, so Satellite creates a new content view version without conflict.

**Config-only re-run** — use `build_sat_disconnected_import.sh --config-only` (see section 6.3). Because `satellite_import_content` is never set, the `content_imports` role is skipped. This is the correct entry point for any change to provisioning templates, operating systems, hostgroups, activation keys, settings, or RBAC after the initial content import is complete.

There is no automation to detect whether content has already been imported or whether new content is available. Operators choose the correct entry point based on whether a new bundle is being delivered.
...
### 8.10 Ansible roles — bundle delivery owns installation

Ansible compliance roles (RedHatOfficial SCAP content, etc.) are installed to `/etc/ansible/roles/`
on the highside satellite by the `bundle_delivery` role during Step 1 of `build_sat_disconnected_import.sh`,
before `main.yml` runs. The `imported_git_repos` role's git clone loop is **skipped** by default on
disconnected satellites — there is no network path to GitHub.

The role reads a single override variable:

| Variable | Default | Effect |
|---|---|---|
| `satellite_imported_git_repos_override` | `false` | When `true`, enables the git clone loop for a highside git server |

Set `satellite_imported_git_repos_override: true` in
`host_vars/<discosatellite>/imported_git_repos.yml` and update the `repository:` URLs to point
to a highside git server (e.g. Gitea on the provisioner node) if cloning from a local source is
required. A `--config-only` pass will apply the change without re-importing content.
...
| `satellite_import_content` | `true` | Set by `build_sat_disconnected_import.sh`; not in inventory. Omit when running `main.yml` directly for config-only changes. |
